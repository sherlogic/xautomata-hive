...
class ApiManager:
    ...
    def __init__(self, root, user, password, ssl_verify: bool = True):
        ...
        self._url_list = False
        self._params_list = False
        self._payload_list = False
    ...
    def execute(self, mode: Literal['GET', 'POST', 'DELETE', 'PUT'], path, headers: Dict = None, single_page: bool = False,
        ...
        def run_request(_mode, _url, _headers, _payload, _params, **_kwargs):
            ...
            if self._return_url_params_payload:
                self._url_list.append(_url)
                self._payload_list.append(_payload)
                self._params_list.append(_params)
...
from hive.cookbook.issue_reports import IssueReports
