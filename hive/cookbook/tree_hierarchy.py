...
class TreeHierarchy(ApiManager):
    ...
    def tree_hierarchy_virtual_domains(self, uuid_customer: str,
        warm_start: bool = False, single_page: bool = False,
        page_size: int = 5000, kwargs: dict = None, **params) -> list:
        """Tree Hierarchy Virtual Domains
        ...
        if kwargs is None:
            kwargs = dict()
        official_params_list = ['sort_by', 'null_fields', 'description',
            'code', 'skip', 'limit', 'like', 'join', 'count']
        params.get('sort_by'), params.get('null_fields'), params.get(
            'description'), params.get('code'), params.get('skip'), params.get(
            'limit'), params.get('like'), params.get('join'), params.get(
            'count')
        if not self._silence_warning:
            warning_wrong_parameters(self.tree_hierarchy_virtual_domains.
                __name__, params, official_params_list)
        response = self.execute('GET', path=
            f'/tree_hierarchy/{uuid_customer}/virtual_domains', single_page
