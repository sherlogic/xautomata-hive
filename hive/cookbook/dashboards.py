...
class Dashboards(ApiManager):
    ...
    def dashboards_customers_create_bulk(self, payload: list,
        single_page: bool = False, page_size: int = 50, kwargs: dict = None,
        **params) -> list:
        """Bulk Link Customers
        ...
        params.get('best_effort')
        if not self._silence_warning:
            warning_wrong_parameters(self.dashboards_customers_create_bulk.
                __name__, params, official_params_list)
        response = self.execute('POST', path=
            f'/dashboards/bulk/create/customers', single_page=single_page,
            page_size=page_size, params=params, payload=payload, **kwargs)
        return response

    def dashboards_customers_delete_bulk(self, payload: list,
        single_page: bool = False, page_size: int = 50, kwargs: dict = None
        ) -> list:
        ...
        if kwargs is None:
            kwargs = dict()
        response = self.execute('POST', path=
            f'/dashboards/bulk/delete/customers', single_page=single_page,
