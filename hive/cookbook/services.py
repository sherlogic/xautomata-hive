...
class Services(ApiManager):
    ...
    def services_last_value(self, warm_start: bool = False,
        single_page: bool = False, page_size: int = 5000,
        kwargs: dict = None, **params) -> list:
        """Read Services Last Value
        ...
        official_params_list = ['sort_by', 'null_fields', 'not_fields',
            'uuid_parent', 'uuid_customer', 'profile', 'name',
            'description', 'status', 'last_value_status', 'skip', 'limit',
            'like', 'join', 'count']
        params.get('sort_by'), params.get('null_fields'), params.get(
            'not_fields'), params.get('uuid_parent'), params.get(
            'uuid_customer'), params.get('profile'), params.get('name'
            ), params.get('description'), params.get('status'), params.get(
            'last_value_status'), params.get('skip'), params.get('limit'
            ), params.get('like'), params.get('join'), params.get('count')
        if not self._silence_warning:
            warning_wrong_parameters(self.services_last_value.__name__,
                params, official_params_list)
        response = self.execute('GET', path=f'/services/last_value',
            single_page=single_page, page_size=page_size, warm_start=
            warm_start, params=params, **kwargs)
        return response

    def services_last_value_extended(self, warm_start: bool = False,
        single_page: bool = False, page_size: int = 5000,
        kwargs: dict = None, **params) -> list:
        """Read Services Last Value Extended
        ...
        if not self._silence_warning:
            warning_wrong_parameters(self.services_last_value_extended.
                __name__, params, official_params_list)
        response = self.execute('GET', path=
            f'/services/last_value_extended', single_page=single_page,
    ...
    def services_last_status_by_automaton_id_query_v2(self,
        warm_start: bool = False, single_page: bool = False,
        page_size: int = 5000, kwargs: dict = None, **params) -> list:
        """Service Query Last Status By Automaton Id V2
        ...
        official_params_list = ['sort_by', 'null_fields', 'uuid_customer',
            ...
            'service_uuid_customer', 'timestamp_start', 'timestamp_end',
            'database_timestamp_start', 'database_timestamp_end', 'status',
            'ranking', 'description', 'fields_to_select', 'skip', 'limit',
            'like', 'join', 'count']
        params.get('sort_by'), params.get('null_fields'), params.get(
            'uuid_customer'), params.get('customer_company_name'), params.get(
            'customer_code'), params.get('customer_status'), params.get(
            ...
            'description'), params.get('fields_to_select'), params.get('skip'
            ), params.get('limit'), params.get('like'), params.get('join'
            ), params.get('count')
        if not self._silence_warning:
            warning_wrong_parameters(self.
                services_last_status_by_automaton_id_query_v2.__name__,
                params, official_params_list)
        response = self.execute('GET', path=
            f'/services/query/last_status/by_automaton_id/v2', single_page=
            single_page, page_size=page_size, warm_start=warm_start, params
            =params, **kwargs)
        return response

    def services_last_status_extended_query(self, warm_start: bool = False,
        single_page: bool = False, page_size: int = 5000,
        kwargs: dict = None, **params) -> list:
        """Service Query Last Status Extended
        ...
        official_params_list = ['extract_data_profile', 'sort_by',
            ...
            'service_description', 'service_status',
            'service_uuid_customer', 'timestamp_start', 'timestamp_end',
            'status', 'ranking', 'description', 'skip', 'limit', 'like',
            'join', 'count']
        params.get('extract_data_profile'), params.get('sort_by'), params.get(
            'null_fields'), params.get('uuid_customer'), params.get(
            'customer_company_name'), params.get('customer_code'), params.get(
            ...
            'timestamp_start'), params.get('timestamp_end'), params.get(
            'status'), params.get('ranking'), params.get('description'
            ), params.get('skip'), params.get('limit'), params.get('like'
            ), params.get('join'), params.get('count')
        if not self._silence_warning:
            warning_wrong_parameters(self.
                services_last_status_extended_query.__name__, params,
                official_params_list)
        response = self.execute('GET', path=
            f'/services/query/last_status_extended', single_page=
            single_page, page_size=page_size, warm_start=warm_start, params
            =params, **kwargs)
        return response

    def services_last_status_extended_query_v2(self,
        warm_start: bool = False, single_page: bool = False,
        page_size: int = 5000, kwargs: dict = None, **params) -> list:
        """Service Query Last Status V2 Extended
        ...
        official_params_list = ['extract_data_profile', 'sort_by',
            ...
            'service_description', 'service_status',
            'service_uuid_customer', 'timestamp_start', 'timestamp_end',
            'status', 'ranking', 'description', 'fields_to_select', 'skip',
            'limit', 'like', 'join', 'count']
        params.get('extract_data_profile'), params.get('sort_by'), params.get(
            'null_fields'), params.get('uuid_customer'), params.get(
            'customer_company_name'), params.get('customer_code'), params.get(
        ...
        if not self._silence_warning:
            warning_wrong_parameters(self.
                services_last_status_extended_query_v2.__name__, params,
                official_params_list)
        response = self.execute('GET', path=
            f'/services/query/last_status_v2_extended', single_page=
            single_page, page_size=page_size, warm_start=warm_start, params
            =params, **kwargs)
        return response
    ...
    def services_dispatchers_create_bulk(self, payload: list,
        single_page: bool = False, page_size: int = 50, kwargs: dict = None,
        **params) -> list:
        ...
        params.get('best_effort')
        if not self._silence_warning:
            warning_wrong_parameters(self.services_dispatchers_create_bulk.
                __name__, params, official_params_list)
        response = self.execute('POST', path=
            f'/services/bulk/create/dispatchers', single_page=single_page,
            page_size=page_size, params=params, payload=payload, **kwargs)
        return response

    def services_dispatchers_delete_bulk(self, payload: list,
        single_page: bool = False, page_size: int = 50, kwargs: dict = None
        ) -> list:
        ...
        if kwargs is None:
            kwargs = dict()
        response = self.execute('POST', path=
            f'/services/bulk/delete/dispatchers', single_page=single_page,
            page_size=page_size, payload=payload, **kwargs)
        return response
