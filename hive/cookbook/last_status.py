...
class LastStatus(ApiManager):
    ...
    def last_status(self, single_page: bool = False, page_size: int = 5000, warm_start: bool = False,
        ...
        Keyword Args:
            ...
            topic (str, optional): additional filter
            last_value_uuid_probe (str, optional): additional filter
            last_value_timestamp (str, optional): additional filter
            last_value_name (str, optional): additional filter
            last_value_value (str, optional): additional filter
            last_value_description (str, optional): additional filter
            last_value_status (Litteral["grey", "purple", "green", "yellow", "red"], optional): additional filter
            last_value_ranking (str, optional): additional filter

        Returns: list
        """
        if kwargs is None: kwargs = dict()
        response = self.execute('GET', path='/last_status/', single_page=single_page, page_size=page_size,
                                warm_start=warm_start, params=params, **kwargs)
        return response

    def last_status_bulk(self, warm_start: bool = False, single_page: bool = False,
